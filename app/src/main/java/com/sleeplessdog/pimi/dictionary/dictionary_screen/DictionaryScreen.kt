...
import com.sleeplessdog.pimi.dictionary.dialogs.GroupDialog
...
import com.sleeplessdog.pimi.dictionary.word_packs.WordPacksOverlay
...
fun DictionaryScreen(
    ...
    renameGroup?.let { (key, title) ->
        GroupDialog(
            onDismiss = { renameGroup = null }, onConfirm = { newName ->
                viewModel.renameGroup(key, newName)
                renameGroup = null
            }, dialogType = DialogType.RENAME_GROUP, groupTitle = title
        )
    }

    deleteGroup?.let { (key, title) ->
        DeletingDialog(
            onDismiss = { deleteGroup = null }, onConfirm = {
                viewModel.deleteGroup(key)
                deleteGroup = null
            }, title = title, dialogType = DialogType.DELETE_GROUP
        )
    }

    if (showSyncOverlay) {
        SyncStateOverlay(syncState = syncState, onRefreshSync = {
            onRefreshSync()
    ...
    Column(
        ...
        HeaderDictionary(
            syncState = syncState, onOpenOverlay = { showSyncOverlay = true })
...
fun HeaderDictionary(
    ...
    if (showWordPacks) {
        WordPacksOverlay(
            viewModel = wordPacksViewModel, onDismiss = { showWordPacks = false })
    }
    Box(
        modifier = Modifier
            .fillMaxWidth()
...
fun DictionaryTabs(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column(
            ...
            PullToRefreshBox(
                isRefreshing = isRefreshing, onRefresh = onRefresh, modifier = Modifier.weight(1f)
            ) {
                when (selectedTab) {
