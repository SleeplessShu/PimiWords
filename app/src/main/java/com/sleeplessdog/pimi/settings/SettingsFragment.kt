...
import com.sleeplessdog.pimi.games.presentation.controller.PimiScrollViewAdapter
...
class SettingsFragment : Fragment(R.layout.settings_fragment) {
    ...
    private fun setupObservers() {
        ...
        vm.difficulty.observe(viewLifecycleOwner) { level ->
            ...
            cardEasy.root.isChecked = (level == DifficultyLevel.EASY)
            cardMedium.root.isChecked = (level == DifficultyLevel.MEDIUM)
            cardHard.root.isChecked = (level == DifficultyLevel.HARD)
            cardExpert.root.isChecked = (level == DifficultyLevel.EXPERT)
        }

        binding.btnShowAllCategories.setOnClickListener {
            showAllTopics = true

            preselected = vm.state.value.featured.filter { it.isSelected }.map { it.key }.toSet()
            showTopicsMenu()
        }
    ...
    private fun renderFeatured(list: List<GroupUiSettings>) {
        ...
        val itemsToShow = if (showAllTopics) list
