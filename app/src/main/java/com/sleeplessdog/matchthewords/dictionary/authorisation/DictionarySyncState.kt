package com.sleeplessdog.matchthewords.dictionary.authorisation
...
fun DictionarySyncState.toSummaryImageRes(): Int {
    if (auth == AuthState.ERROR || globalDb == DataTransferStatus.ERROR || userDb == DataTransferStatus.ERROR)
    ...
    if (globalDb == DataTransferStatus.DOWNLOADING || userDb == DataTransferStatus.DOWNLOADING ||
        globalDb == DataTransferStatus.UPLOADING || userDb == DataTransferStatus.UPLOADING)
        return R.drawable.ic_cloud_download
    if (globalDb == DataTransferStatus.REFRESHING || userDb == DataTransferStatus.REFRESHING ||
        globalDb == DataTransferStatus.DEPLOYING || userDb == DataTransferStatus.DEPLOYING)
        return R.drawable.ic_cloud_refreshing
    if (globalDb == DataTransferStatus.SUCCESS || globalDb == DataTransferStatus.ASSETS ||
        globalDb == DataTransferStatus.NETWORK)
        return R.drawable.ic_cloud_success
